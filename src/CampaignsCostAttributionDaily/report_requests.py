...
PREFIX = 'CampaignsCostAttributionDaily/'
RESULT_FILE_NAME='CampaignsCostAttributionDaily.' + CURRENT_TIME + '.' + REPORT_FILE_FORMAT.lower()
...
def get_report_request(account_id,reporting_service,DateRangeLastDate):
    ...
    time.CustomDateRangeEnd.Day=(date.today() - timedelta(days=2)).day
    time.CustomDateRangeEnd.Month=(date.today() - timedelta(days=2)).month
    time.CustomDateRangeEnd.Year=(date.today() - timedelta(days=2)).year
    time.CustomDateRangeStart.Day=(datetime.strptime(DateRangeLastDate,'%Y-%m-%d') +  timedelta(days=1)).day
    time.CustomDateRangeStart.Month=(datetime.strptime(DateRangeLastDate,'%Y-%m-%d') +  timedelta(days=1)).month
    time.CustomDateRangeStart.Year=(datetime.strptime(DateRangeLastDate,'%Y-%m-%d') +  timedelta(days=1)).year
...
def main_report_requets():
    ...
    try:
        ...
        DateRangeLastDate = GHWM.get_highwatermark(os.environ['targetbucket'],PREFIX,'2020-08-30')
        print(DateRangeLastDate)
        print((date.today() - timedelta(days=2)).strftime("%Y-%m-%d"))
        if (DateRangeLastDate > (date.today() - timedelta(days=2)).strftime("%Y-%m-%d")):
            return 
        ...
        PTS.push_to_s3(FILE_DIRECTORY,PREFIX,RESULT_FILE_NAME,(date.today() - timedelta(days=2)).strftime("%Y-%m-%d"))
